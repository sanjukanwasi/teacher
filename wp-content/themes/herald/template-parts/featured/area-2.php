<?php $color_class = herald_get_option('lay_fa2_color') ? 'herald-fa-colored' : ''  ?>

<div class="herald-fa-wrapper herald-fa-2 <?php echo esc_attr($color_class); ?>">
		  ...
		  <?php while($fa_query->have_posts()): $fa_query->the_post(); ?>
			  ...
			  <article class="herald-fa-item herald-cat-<?php echo esc_attr($cat_id); ?>">	    
				    ...
				    <header class="entry-header">				
						...
						<?php if( herald_get_option( 'lay_fa2_cat' ) ) : ?>
				    	...
				    	<?php the_title( sprintf( '<h2 class="entry-title h6"><a href="%s">'.herald_post_format_icon(), esc_url( get_permalink() ) ), '</a></h2>' ); ?>
				      	...
				      	<?php if( $meta = herald_get_meta_data( 'fa2' ) ) : ?>
							<div class="entry-meta"><?php echo $meta; ?></div>
						<?php endif; ?>
					  	
					  	<?php if( herald_get_option('lay_fa2_excerpt') ) : ?>
							<div class="entry-content">
								<?php echo herald_get_excerpt( 'fa2' ); ?>
							</div>
						<?php endif; ?>
